 ...
 *
 */
public class Plotter6167i extends JInternalFrame {
	...
	private class ControlPanel extends JInternalFrame {
		 ...
		 * 
		 * @param string The text to appear in the title bar.
		 */
...
//				}
	...
	public class PlotterPane extends JPanel
		 ...
		 */
		public PlotterPane() {
			this.setPreferredSize(Plotter6167i.this.getPlotterPaneDefaultDimension());
			this.setBackground(Color.WHITE);
			this.insets = Plotter6167i.this.insets;
...
//			this.add(this.captionArea);
		...
		private void drawXTickLabels(final Graphics2D g, final PathIterator pathIterator2, final int sigDig) {
			...
			while (!pathIterator2.isDone()) {
				...
				final var tickText = Plotter6167i.this.xDateOrigin == Long.MIN_VALUE
						? String.format(format, coords[0]).replaceFirst("(\\.\\d*)0+($|e)", "$1$2")
						: Plotter6167i.getDateinstance().format(new Date(
								Plotter6167i.this.xDateOrigin + (long) coords[0] * Plotter6167i.MILLIS_PER_DAY));
		...
		private void drawYTickLabels(final Graphics2D g, final PathIterator pathIteratorY, final int sigDig) {
			while (!pathIteratorY.isDone()) {
				...
				final var format = "%." + String.valueOf(sigDig) + "g";
				final var yLabelValue = Plotter6167i.this.semiLog ? Math.pow(1e1, coords[1]) : coords[1];
...
//		}
		...
		private AffineTransform getPlotTransform(final double sx2, final double sy2, final AffineTransform pan2) {
			...
			if (Plotter6167i.this.pathToUnitBoxTransform == null) {
				throw new RuntimeException("pathToUnitBoxTransform is null");
			} else {
				plotTransform.concatenate(Plotter6167i.this.pathToUnitBoxTransform);
			}
			...
			if (this.markers != null) {
				...
				for (final Shape marker : this.markers) {
					if (marker != null && point != null && marker.contains(point)) {
						...
						final var tipTextAvailable = Plotter6167i.this.tips != null
								&& Plotter6167i.this.tips.size() > index && Plotter6167i.this.tips.get(index) != null;
						this.setToolTipText(
								tipTextAvailable ? Plotter6167i.this.tips.get(index) : String.format("%,d", index));
		...
		public void paint(final Graphics g0) {
			...
			this.insets = new Insets(Plotter6167i.this.top, Plotter6167i.this.left, Plotter6167i.this.bottom,
					Plotter6167i.this.right);
		...
		private void painter(final Graphics2D g, final int width, final int height) {
			try {
				...
				if (Plotter6167i.this.isShowCaption()) {
					this.add(this.captionArea);
				}
				...
				if (this.path != null || true) {
					...
					final var transformedXGrid = this.plotTransform.createTransformedShape(Plotter6167i.this.xGrid);
					final var transformedXGrid2 = this.plotTransform.createTransformedShape(Plotter6167i.this.xGrid2);
					final var transformedXGrid3 = this.plotTransform.createTransformedShape(Plotter6167i.this.xGrid3);
					final var transformedYGrid = this.plotTransform.createTransformedShape(Plotter6167i.this.yGrid);
					final var transformedYGrid2 = this.plotTransform.createTransformedShape(Plotter6167i.this.yGrid2);
					final var transformedYGrid3 = this.plotTransform.createTransformedShape(Plotter6167i.this.yGrid3);

					final var oldColor = g.getColor();
					g.setColor(Color.BLACK);

					final var xBounds = Plotter6167i.this.xGrid.getBounds2D();
					...
					this.drawXTickLabels(g, Plotter6167i.this.xGrid2.getPathIterator(null), sigDigX);

					if (Plotter6167i.this.xTicks2 != null) {
						if (Plotter6167i.this.xTicks2.length < 2) {
							this.drawXTickLabels(g, Plotter6167i.this.xGrid3.getPathIterator(null), sigDigX);
						}
						if (Plotter6167i.this.xTicks2.length + Plotter6167i.this.xTicks3.length < 2) {
							this.drawXTickLabels(g, Plotter6167i.this.xGrid.getPathIterator(null), sigDigX);
						}
					}

					final var yBounds = Plotter6167i.this.yGrid.getBounds2D();
					...
					final var sigDigY = (int) (2.4 + Math.log10(maxAbsY / yRange));// significant digits for Y tick
																					// labels
					this.drawYTickLabels(g, Plotter6167i.this.yGrid2.getPathIterator(null), sigDigY);
					if (Plotter6167i.this.yTicks2 != null) {
						if (Plotter6167i.this.yTicks2.length < 2) {
							this.drawYTickLabels(g, Plotter6167i.this.yGrid3.getPathIterator(null), sigDigY);
						}
						if (Plotter6167i.this.yTicks2.length + Plotter6167i.this.yTicks3.length < 2) {
							this.drawYTickLabels(g, Plotter6167i.this.yGrid.getPathIterator(null), sigDigY);
						}
					}
				...
				g.setClip(null);
				if (Plotter6167i.this.textItems != null) {
					final var iterator = Plotter6167i.this.textItems2.iterator();
					while (iterator.hasNext()) {
						final var next = iterator.next();
						next.plotText(g, this.plotTransform);
					}
				}
				if (Plotter6167i.this.plotTitle != null && !Plotter6167i.this.plotTitle.isEmpty()) {
					this.plotTheTitle(g, w);
				}

				for (final LegendItem li : Plotter6167i.this.legendItems) {
...
//					area.setOpaque(true);
		...
		private void plotTheTitle(final Graphics2D g, final int w) {
			...
			var font = Plotter6167i.this.titleFont;
			var count = 0;
			do {
				++count;
				if (!fitDone) {
				...
				stringBounds = fontMetrics.getStringBounds(Plotter6167i.this.plotTitle, g);
				x = w / 2 - stringBounds.getCenterX() + Plotter6167i.this.left;
				fitDone = count > 100
						|| x > 0 && x + stringBounds.getWidth() < Plotter6167i.this.left + w + Plotter6167i.this.right;
			} while (!fitDone);
			final var y = Plotter6167i.this.top / 2 - stringBounds.getCenterY();
			final var oldFont = g.getFont();
			final var oldColor = g.getColor();
			g.setFont(font);
			g.setColor(Plotter6167i.this.titleColor);
			g.drawString(Plotter6167i.this.plotTitle, (float) x, (float) y);
			g.setColor(oldColor);
			g.setFont(oldFont);
		}
...
//				this.s = 1;
				 ...
				 */
				Plotter6167i.this.createGrid(this.dataBounds, true);
...
//				final AffineTransform plotTransform2 = this.getPlotTransform(this.s, this.pan);
				...
				if (Plotter6167i.roundrangeDebug) {
	...
	public static DateFormat getDateinstance() {
		return Plotter6167i.dateInstance;
	}
	...
	public static boolean isRoundrangeDebug() {
		return Plotter6167i.roundrangeDebug;
	}
	...
	public static void setRoundrangeDebug(final boolean roundrangeDebug) {
		Plotter6167i.roundrangeDebug = roundrangeDebug;
	}
...
//	private Dimension plotterPaneDefaultDimension = new Dimension(865, 600);
	...
	/**
	 * @param arg0 The text to appear in the title bar.
	 */
	public Plotter6167i(final String arg0) {
		super(arg0);
		this.init();
	}
	...
	public void addPlotPath(final Stroke stroke, final Shape path, final Color color, final Color fillColor) {
		if (this.plotterPane.path == null) {
			...
			newButton.addActionListener(e -> Plotter6167i.this.plotterPane.repaint());
			newButton.setUI(Plotter6167i.newUI);
	...
	protected void copyImageToFile() throws IOException {
		...
		final var outFileName = Plotter6167i.getTimeStamp() + Plotter6167i.this.getTitle() + ".png";
		...
		if (outFileName2 == null|| outFileName2.isBlank()) {
			return;
		}
	...
	private void createGrid(final Rectangle2D gridBounds, final boolean zooming) {
		if (Plotter6167i.roundrangeDebug) {// if debugging, print message with unique id "81930de4"
		...
		try {
			 ...
			 */
			final var xRoundRange = Plotter6167i.getRoundRange(gridBounds.getX(),
					gridBounds.getX() + gridBounds.getWidth());
			final var yRoundRange = Plotter6167i.getRoundRange(gridBounds.getY(),
...
//			}
	...
	private void init() {
		...
		menuItemSave.addActionListener(arg0 -> {
			try {
				Plotter6167i.this.copyImageToFile();
		...
		menuItemManagePlots.addActionListener(e -> {
			...
			if (this.controlPanel.getDesktopPane() == null) {
				 ...
				 */
				final var desktopPane2 = Plotter6167i.this.getDesktopPane();
...
//					this.controlPanel.getContentPane().setLayout(new FlowLayout());
		...
		menuItemQuit.addActionListener(arg0 -> Plotter6167i.this.shutdown());
