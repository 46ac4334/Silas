 ...
 *
 */
public class Plotter6168i extends JInternalFrame {
...
//		public List<JToggleButton> toggleButtons = new ArrayList<>();
		 ...
		 */
		public PlotterPane() {
			this.setPreferredSize(Plotter6168i.this.getPlotterPaneDefaultDimension());
			this.setBackground(Color.WHITE);
			this.insets = Plotter6168i.this.insets;
...
//			this.add(this.captionArea);
		...
		private void drawXTickLabels(final Graphics2D g, final PathIterator pathIterator2, final int sigDig) {
			...
			while (!pathIterator2.isDone()) {
				...
				final String tickText = Plotter6168i.this.xDateOrigin == Long.MIN_VALUE
						? String.format(format, coords[0]).replaceFirst("(\\.\\d*)0+($|e)", "$1$2")
						: Plotter6168i.getDateinstance().format(new Date(
								Plotter6168i.this.xDateOrigin + ((long) coords[0] * Plotter6168i.MILLIS_PER_DAY)));
		...
		private void drawYTickLabels(final Graphics2D g, final PathIterator pathIteratorY, final int sigDig) {
			while (!pathIteratorY.isDone()) {
				...
				final String format = "%." + String.valueOf(sigDig) + "g";
				final double yLabelValue = Plotter6168i.this.semiLog ? Math.pow(1e1, coords[1]) : coords[1];
...
//		}
		...
		private AffineTransform getPlotTransform(final double sx2, final double sy2, final AffineTransform pan2) {
			final AffineTransform plotTransform = new AffineTransform(
					this.transform == null ? new AffineTransform() : this.transform);
			if (Plotter6168i.this.pathToUnitBoxTransform == null) {
				StaticMethods.stacktraceAll("pathToUnitBoxTransform is null");
			} else {
				plotTransform.concatenate(Plotter6168i.this.pathToUnitBoxTransform);
			}
			...
			if (this.markers != null) {
				...
				for (final Shape marker : this.markers) {
					if ((marker != null) && (point != null) && marker.contains(point)) {
						...
						final boolean tipTextAvailable = ((Plotter6168i.this.tips != null)
								&& (Plotter6168i.this.tips.size() > index)
								&& (Plotter6168i.this.tips.get(index) != null));
						this.setToolTipText(
								tipTextAvailable ? Plotter6168i.this.tips.get(index) : String.format("%,d", index));
		...
		public void paint(final Graphics g0) {
			...
			this.insets = new Insets(Plotter6168i.this.top, Plotter6168i.this.left, Plotter6168i.this.bottom,
					Plotter6168i.this.right);
		...
		private void painter(final Graphics2D g, final int width, final int height) {
			try {
				...
				if (Plotter6168i.this.isShowCaption()) {
					this.add(this.captionArea);
				}
				...
				if ((this.path != null) || true) {
					...
					final Shape transformedXGrid = this.plotTransform.createTransformedShape(Plotter6168i.this.xGrid);
					final Shape transformedXGrid2 = this.plotTransform.createTransformedShape(Plotter6168i.this.xGrid2);
					final Shape transformedXGrid3 = this.plotTransform.createTransformedShape(Plotter6168i.this.xGrid3);
					final Shape transformedYGrid = this.plotTransform.createTransformedShape(Plotter6168i.this.yGrid);
					final Shape transformedYGrid2 = this.plotTransform.createTransformedShape(Plotter6168i.this.yGrid2);
					final Shape transformedYGrid3 = this.plotTransform.createTransformedShape(Plotter6168i.this.yGrid3);

					final Color oldColor = g.getColor();
					g.setColor(Color.BLACK);

					final Rectangle2D xBounds = Plotter6168i.this.xGrid.getBounds2D();
					...
					this.drawXTickLabels(g, Plotter6168i.this.xGrid2.getPathIterator(null), sigDigX);

					if (Plotter6168i.this.xTicks2 != null) {
						if (Plotter6168i.this.xTicks2.length < 2) {
							this.drawXTickLabels(g, Plotter6168i.this.xGrid3.getPathIterator(null), sigDigX);
						}
						if ((Plotter6168i.this.xTicks2.length + Plotter6168i.this.xTicks3.length) < 2) {
							this.drawXTickLabels(g, Plotter6168i.this.xGrid.getPathIterator(null), sigDigX);
						}
					}

					final Rectangle2D yBounds = Plotter6168i.this.yGrid.getBounds2D();
					...
					final int sigDigY = (int) (2.4 + Math.log10(maxAbsY / yRange));// significant digits for Y tick
																					// labels
					this.drawYTickLabels(g, Plotter6168i.this.yGrid2.getPathIterator(null), sigDigY);
					if (Plotter6168i.this.yTicks2 != null) {
						if (Plotter6168i.this.yTicks2.length < 2) {
							this.drawYTickLabels(g, Plotter6168i.this.yGrid3.getPathIterator(null), sigDigY);
						}
						if ((Plotter6168i.this.yTicks2.length + Plotter6168i.this.yTicks3.length) < 2) {
							this.drawYTickLabels(g, Plotter6168i.this.yGrid.getPathIterator(null), sigDigY);
						}
					}
				...
				g.setClip(null);
				if (Plotter6168i.this.textItems != null) {
					final Iterator<TextItem> iterator = Plotter6168i.this.textItems2.iterator();
					while (iterator.hasNext()) {
						final TextItem next = iterator.next();
						next.plotText(g, this.plotTransform);
					}
				}
				if ((Plotter6168i.this.plotTitle != null) && !Plotter6168i.this.plotTitle.isEmpty()) {
					this.plotTheTitle(g, w);
				}

				for (final LegendItem li : Plotter6168i.this.legendItems) {
...
//					area.setOpaque(true);
		...
		private void plotTheTitle(final Graphics2D g, final int w) {
			...
			Font font = Plotter6168i.this.titleFont;
			int count = 0;
			do {
				++count;
				if (!fitDone) {
				...
				stringBounds = fontMetrics.getStringBounds(Plotter6168i.this.plotTitle, g);
				x = ((w / 2) - stringBounds.getCenterX()) + Plotter6168i.this.left;
				fitDone = (count > 100) || ((x > 0)
						&& ((x + stringBounds.getWidth()) < (Plotter6168i.this.left + w + Plotter6168i.this.right)));
			} while (!fitDone);
			final double y = (Plotter6168i.this.top / 2) - stringBounds.getCenterY();
			final Font oldFont = g.getFont();
			final Color oldColor = g.getColor();
			g.setFont(font);
			g.setColor(Plotter6168i.this.titleColor);
			g.drawString(Plotter6168i.this.plotTitle, (float) x, (float) y);
			g.setColor(oldColor);
			g.setFont(oldFont);
		}
...
//				this.s = 1;
				 ...
				 */
				Plotter6168i.this.createGrid(this.dataBounds, true);
...
//				final AffineTransform plotTransform2 = this.getPlotTransform(this.s, this.pan);
				...
				if (Plotter6168i.roundrangeDebug) {
	...
	public static DateFormat getDateinstance() {
		return Plotter6168i.dateInstance;
	}
	...
	public static boolean isRoundrangeDebug() {
		return Plotter6168i.roundrangeDebug;
	}
	...
	public static void setRoundrangeDebug(final boolean roundrangeDebug) {
		Plotter6168i.roundrangeDebug = roundrangeDebug;
	}
	 ...
	 * @param arg0
	 */
	public Plotter6168i(final String arg0) {
		super(arg0);
		this.init();
	}
	...
	public void addPlotPath(final Stroke stroke, final Shape path, final Color color, final Color fillColor) {
		if (this.plotterPane.path == null) {
			...
			newButton.addActionListener(e -> Plotter6168i.this.plotterPane.repaint());
			newButton.setUI(Plotter6168i.newUI);
	...
	protected void copyImageToFile() throws IOException {
		...
		final String outFileName = Plotter6168i.getTimeStamp() + Plotter6168i.this.getTitle() + ".png";
	...
	private void createGrid(final Rectangle2D gridBounds, final boolean zooming) {
		if (Plotter6168i.roundrangeDebug) {// if debugging, print message with unique id "81930de4"
		...
		try {
			 ...
			 */
			final int[] xRoundRange = Plotter6168i.getRoundRange(gridBounds.getX(),
					gridBounds.getX() + gridBounds.getWidth());
			final int[] yRoundRange = Plotter6168i.getRoundRange(gridBounds.getY(),
...
//			}
	...
	private void init() {
		...
		menuItemSave.addActionListener(arg0 -> {
			try {
				Plotter6168i.this.copyImageToFile();
		...
		menuItemManagePlots.addActionListener(e -> {
			...
			if (this.controlPanel.getDesktopPane() == null) {
				 ...
				 */
				final JDesktopPane desktopPane2 = Plotter6168i.this.getDesktopPane();
...
//					this.controlPanel.getContentPane().setLayout(new FlowLayout());
		...
		menuItemQuit.addActionListener(arg0 -> Plotter6168i.this.shutdown());
